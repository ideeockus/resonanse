...
pub async fn handle_set_city(bot: Bot, dialogue: MyDialogue, msg: Message) -> HandlerResult {
    ...
    let chosen_city = match msg.text() {
        None => {
            let message = bot.send_message(msg.chat.id, reject_user_input_msg);
            message.await?;
            return Ok(());
        }
        Some(msg_text) => {
    ...
    let user_id = accounts_repo
        ...
        .await?;
    accounts_repo
        .set_user_city(user_id, chosen_city.to_string())
        .await?;
    dialogue.update(BaseState::Idle).await?;

    let mut message = bot.send_message(msg.chat.id, format!("Выбран город: {}", chosen_city,));
...
pub async fn handle_set_description(bot: Bot, dialogue: MyDialogue, msg: Message) -> HandlerResult {
    // 0. check user exists
    let user = msg.from();
    let accounts_repo = ACCOUNTS_REPOSITORY
        .get()
