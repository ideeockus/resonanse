...
pub async fn handle_get_digest_command(bot: Bot, dialogue: MyDialogue, msg: Message) -> HandlerResult {
    // 1. send rpc request
    ...
    let rec_service_client = RecServiceClient::new(&rabbitmq_host).await;
    let user_id = msg.chat.id.0;

    // todo  get user_id by tg_user_id
    let recommendation = rec_service_client.rpc_get_recommendation_by_user(
        user_id,
    ).await?;
