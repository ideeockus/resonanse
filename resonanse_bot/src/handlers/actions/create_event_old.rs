...
//     datetime_from: Option<chrono::NaiveDateTime>,
...
//             event_title=markdown::escape(&self.title),
//             event_description=markdown::escape(&self.description),
//             event_subject=markdown::escape(&self.subject.to_string()),
//             event_datetime=markdown::escape(&self.datetime_from.format(DEFAULT_DATETIME_FORMAT).to_string()),
//             event_location=match &self.location.title.as_deref() {
//                 None => "".to_string(),
//                 Some(location_title) => format!("📍 Место: _{}_", markdown::escape(location_title)),
//             },
//             event_contact_info=match &self.contact_info.as_deref() {
//                 None => "".to_string(),
...
//             datetime_from: value.datetime_from.unwrap_or(chrono::Local::now().naive_local()),
//             // timezone: chrono_tz::Tz::Europe__Moscow,
//             datetime_to: value.datetime_to.unwrap_or(chrono::Local::now().naive_local()),
//             location: value.geo_position.unwrap_or_else(|| {
//                 warn!("cannot set event for BaseEvent from FillingEvent");
//                 Location::from_ll(0.0, 0.0)
//             }),
//             location_title: "".to_string(),
...
//     };
//
//     filling_event.datetime_from = Some(event_dt);
//
//     dialogue
//         .update(BaseState::CreateEvent {
...
//         msg.chat.id,
//         "Отправьте Геометку (Прикрепить вложение -> локация). Или ссылку в Yandex.Map (не сокращенную, в ней должны быть указаны координаты)",
//     );
//     message.await?;
//
//     Ok(())
...
//                 Some(loc) => loc,
//                 None => {
//                     reject_user_answer!(bot, msg.chat.id, "Координаты в ссылке не найдены");
//                 }
//             }
//         }
//         _ => {
